...
const addMatchForm = {
	...
	gameLengthInput: tryGetElement<HTMLInputElement>('#game-length', addMatchSection),
...
const matchList: NamedElement = {
	node: tryGetElement<HTMLElement>('#match-list'),
	name: 'matchList'
}
...
function makeEditable(matchId: number) {
	...
	const sideSpan = durationSpan.cloneNode() as HTMLSpanElement;
	const resultSpan = durationSpan.cloneNode() as HTMLSpanElement;
	...
	const direRadio = radiantRadio.cloneNode() as HTMLInputElement;
	...
	const lossRadio = winRadio.cloneNode() as HTMLInputElement;
